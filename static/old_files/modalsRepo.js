...
export function selectRow(row, id, service, epg, donationInfo, reddit_user) {
  ...
  document.getElementById("PreviewList").addEventListener("click", () => {
    const m3uUrl = `${window.location.origin}/m3u/${id}`; // URL absoluta
    const epgUrl = document.getElementById("EPG").value;

    window.location.href = `${window.location.origin}/epgdisplayer?m3u=${encodeURIComponent(m3uUrl)}&epg=${encodeURIComponent(epgUrl)}`;
  });
  ...
  .then(data => {
    ...
    title.textContent = "Categories auto-updating";
    ...
    data.groups.forEach(group => {
      ...
      // Text visible ("Yes" o "No") en comptes de checkbox
      const textValue = document.createElement('span');
      textValue.textContent = sortValue === '1' ? 'Yes ✅' : 'No ❌';
      checkboxCell.appendChild(textValue);
