...
document.getElementById("AutomaticProcess").addEventListener("click", () => {
  ...
  export function loadCSV() {
    ...
    .then(data => {
      ...
      tileContainer.innerHTML = ""; // Neteja contingut anterior

      // Ordenació segons l'opció seleccionada
      data.sort((a, b) => {
        switch (orderBy) {
          case "Downloads":
            return b.clicks - a.clicks;
          case "LastModified":
            const [ddA, mmA, yyyyA] = a.timestamp.split('/');
            const [ddB, mmB, yyyyB] = b.timestamp.split('/');
            const dateA = new Date(`${yyyyA}-${mmA}-${ddA}`);
            const dateB = new Date(`${yyyyB}-${mmB}-${ddB}`);
            return dateB - dateA;
          case "Service":
            return a.service_name.localeCompare(b.service_name);
          case "ListOwner":
            return a.reddit_user.localeCompare(b.reddit_user);
          case "CaseID":
            default:
              return a.id - b.id;
        }
      ...
      data.forEach(row => {
        ...
        tile.onclick = () => {
          selectRow(tile, row.id, row.service_name, row.github_epg_url, row.donation_info, row.reddit_user);
        };
...
export function submitSelectedGroups() {
 ...
 const postData = {
  ...
  .then(response => {
        ...
        // 👉 Llegim el filename del header enviat pel backend
        const disposition = response.headers.get("Content-Disposition");
        let filename = `modified_playlist_${selectedID}.m3u`; // fallback
  ...
  export function editCredentials(formData) {
    ...
    .then(async response => {
      ...
      // Amaga spinners tan aviat com rebem resposta
      if (spinner) spinner.style.display = "none";
      if (waitEl) waitEl.style.display = "none";

      if (!response.ok) {
        const message = data && data.error ? data.error : 'Unknown server error';
        // Mostra l'error a l'UI i llença perquè caigui al catch si cal
      ...
      // Resposta OK
      if (data && data.error) {
        // Backend ha marcat error dins 200 OK (cas improbable si usaves 404/4xx)
        showStatus(data.error, "error", true);
      } else {
        // Èxit: mostra "Success"
        showStatus(`✅ Success: credentials updated`, "success");
    ...
    .catch(error => {
      ...
      // Mostra l'error a la UI
      showStatus(error.message || 'Network error', "error", true);

      // Opcional: alerta emergent a l'usuari
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  orderWrapper.id = "orderWrapperID";

  // Etiqueta de text
  const label = document.createElement("span");
  label.textContent = "ORDER LISTS BY:";
  label.id= "SpanOrderBY"

  const dropdown = document.createElement("select");
  dropdown.id = "orderBy";
  dropdown.style.marginBottom = "1em";
  dropdown.innerHTML = `
    <option value="CaseID" selected>ID</option>
    <option value="Downloads">Downloads</option>
    <option value="LastModified">Last Modified Date</option>
    <option value="Service">Service</option>
    <option value="ListOwner">List Owner</option>
  `;

  orderWrapper.appendChild(label);
  orderWrapper.appendChild(dropdown);
  controls.insertBefore(orderWrapper, controls.firstChild);

  dropdown.addEventListener("change", () => {
    loadCSV(); // Torna a carregar i ordenar
  });
  loadCSV();
});
