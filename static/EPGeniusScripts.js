...
async function uploadToGoogleDrive(accessToken, blob, filename, listID) {
  const metadata = {
    name: filename,
  ...
  if (!uploadRes.ok || !fileId) throw new Error(uploadJson?.error?.message || 'Drive upload failed');
  ...
  return { fileId, downloadLink: `https://drive.usercontent.google.com/download?id=${fileId}&confirm=t` };
}
...
document.getElementById('uploadDriveBtn').addEventListener('click', async function () {
  ...
  try {
    // IMPORTANT: do this FIRST so iOS will allow the Google popup
    ...
    // Now upload WITHOUT triggering auth UI again
    const result = await uploadToGoogleDrive(accessToken, blob, filename, selectedID);
